...
therules[0] = ['A', 'C+F+A+F+C+B+A']; // first rule
therules[1] = ['B', '+F+C+F+F+F+C+']; // second rule
//therules[2] = ['C', '++A']; // second rule

// plus means left and minus means right, F draws forward
...
function setup()
{
  ...
  stroke(31, 148, 31, 255); // draw in black
...
function drawIt(k)
{
  ...
  fill(255);
  noStroke;
  text("b o u q u e t", height/3.5, width/4.5)
  
  fill(0, 255, 0, 0); // red unless i change it
  if(k=='F') // draw forward
  {
    ...
    triangle(x, y, x1, y1);
    line(x1 * 2, y1 * 2); // connect the old and the new
